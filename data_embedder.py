...
class DataEmbedder:
    ...
    def __init__(self):
        ...
        # )
        # added
        self.embedding_function = OllamaEmbeddings(model=Config.OLLAMA_MODEL)
    ...
    def _initialize_chroma(self) -> None:
        ...
        try:
            ...
            try:
                ...
                # embedding_function is not required with Ollama
    ...
    def embed_data(
        ...
        try:
            ...
            self.logger.info(f"Embed Data: File {source_name}")
            ...
            # self.logger.info(f"Embed Data: File Hashes {source_name}")
            ...
            for start_idx in range(0, len(df), batch_size):
                ...
                # if start_idx%10==0:
                #     self.logger.info(f"Batch {start_idx}: {documents[0]} - {metadatas[0]} - {ids[0]}")

                self.collection.add(documents=documents, metadatas=metadatas, ids=ids)
                # self.logger.info("Checking List of collections")

            
            # self.logger.info(f"""{[c.name for c in self.collection.list_collections()]}""")
            return True
    ...
    def _prepare_batch_data(
        ...
        for idx, row in df.iterrows():
            ...
            # self.logger.info(f"_prepare_batch_data {source_name}")
        ...
        # self.logger.info(f"_prepare_batch_data documents {documents[0]}")    
        # self.logger.info(f"_prepare_batch_data metadatas {metadatas[0]}")
        # self.logger.info(f"_prepare_batch_data ids {ids[0]}") 
    ...
    def _prepare_row_description(self, row: pd.Series, df: pd.DataFrame) -> str:
        ...
        parts = []
        # self.logger.info("_prepare_row_description")
        for col in df.columns:
            # Format date columns
